...
namespace Diamond.Core.Extensions.DependencyInjection
{
	...
	public class ServicesConfigurationFileProvider : FileConfigurationProvider
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="ServicesConfigurationFileProvider"/> class using the specified <see
		/// cref="ServicesConfigurationFileSource"/>.
		/// </summary>
		...
		/// langword="null"/>.</param>
		public ServicesConfigurationFileProvider(ServicesConfigurationFileSource source)
			: base(source)
		{
		}
		...
		{
			//
			// Get a FileInfo object to convert relative paths to full paths.
			//
			FileInfo file = new(this.Source.Path);
			...
			if (file.Exists)
			{
				...
				//
				IDictionary<string, string> result = ServicesConfigurationFileParser.Parse(0, json);
			...
			else
			{
				if (!this.Source.Optional)
				{
					...
					//
					throw new DirectoryNotFoundException($"The configuration services path '{file.FullName}' was not found.");
				}
			}
		}
